...
def describe_event(event: SurveillanceEvent) -> str:
    ...
    if event.event_type == "FISHING_PATTERN" and p.get("window_start") and p.get("window_end"):
        start = datetime.fromisoformat(p["window_start"]).strftime("%H:%M")
        end = datetime.fromisoformat(p["window_end"]).strftime("%H:%M")
        return f"Fishing pattern score {event.score:.0f}/100 between {start} and {end}"
