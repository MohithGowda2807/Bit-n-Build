...
def assign_case(case_id: int, payload: AssignRequest, db: Session = Depends(get_db)):
    _load(db, case_id)
    return _detail(db, RiskService(db).assign_case(case_id, payload.assignee, payload.actor))
...
def escalate_case(case_id: int, payload: CaseActionRequest, db: Session = Depends(get_db)):
    _load(db, case_id)
    return _detail(db, RiskService(db).escalate_case(case_id, payload.actor, payload.note))
...
def resolve_case(case_id: int, payload: CaseActionRequest, db: Session = Depends(get_db)):
    _load(db, case_id)
    return _detail(db, RiskService(db).resolve_case(case_id, payload.actor, payload.note))
...
def dismiss_case(case_id: int, payload: DismissRequest, db: Session = Depends(get_db)):
    ...
    return _detail(db, RiskService(db).dismiss_case(case_id, payload.reason, payload.actor, payload.note))
...
def analyze_case(case_id: int, db: Session = Depends(get_db)):
    ...
    case.add_audit("AGENT_ANALYSIS", "investigation_agent", f"narrative generated by {result.provider} ({result.model})")
    db.commit()
