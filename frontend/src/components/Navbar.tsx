...
import { Compass, Waves, ShieldCheck, Radio } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ analytics, systemStatus, wsConnected = false }) => {
  return (
    <header className="bg-[#071224]/95 backdrop-blur border-b border-slate-800 px-6 py-3 flex items-center justify-between sticky top-0 z-50">
      <div className="flex items-center space-x-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400">Autonomous Maritime Multi-Agent Command Platform</p>
        </div>
      </div>
      ...
      <div className="hidden lg:flex items-center space-x-6 text-xs font-mono">
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-lg px-3 py-1.5 flex items-center space-x-2">
          ...
          <span className="text-emerald-400 font-bold">4 SPECIALISTS + ORCHESTRATOR</span>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="flex items-center space-x-1.5 bg-slate-900/90 border border-slate-800 px-3 py-1.5 rounded-full">
          <Radio className={`w-3.5 h-3.5 ${wsConnected ? 'text-cyan-400 animate-pulse' : 'text-slate-500'}`} />
          <span className="text-[11px] font-mono text-slate-300">
            {wsConnected ? 'Telemetry Live' : 'Polling Sync'}
          </span>
        </div>
