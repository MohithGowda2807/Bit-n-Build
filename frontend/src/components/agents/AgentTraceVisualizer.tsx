...
export const AgentTraceVisualizer: React.FC<AgentTraceVisualizerProps> = ({
      ...
      {domainImpact && (
        <div className="grid grid-cols-3 gap-2.5 p-3 bg-os-void border border-os-signal rounded-row">
          <div className="p-2 bg-os-panel rounded-input">
            ...
            <div className="text-[10px] text-os-ash">
 ...
 Correlated Anomaly Verified
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-[11px] text-os-ash font-bold uppercase pb-1 border-b border-os-pewter">
 ...
 return (
              <div
 key={idx}
 className="border border-os-pewter hover:border-os-pewter bg-os-void rounded-row overflow-hidden transition"
              >
                {/* Step Header */}
                <div
 onClick={() => setExpandedIndex(isExpanded ? null : idx)}
 className="p-3 cursor-pointer flex items-center justify-between hover:bg-os-panel"
                >
                  <div className="flex items-center space-x-2.5 min-w-0">
                    <span className="w-5 h-5 rounded-full bg-os-raised border border-os-pewter text-[10px] flex items-center justify-center font-bold text-os-fog shrink-0">
                      {trace.step || idx + 1}
                    </span>
                  ...
                  <div className="flex items-center space-x-2 shrink-0 text-[10px] text-os-ash">
                    ...
                    <span className="text-os-slate">{isExpanded ? '▲' : '▼'}</span>
                  </div>
                </div>

                {/* Expanded Details */}
                {isExpanded && (
                  <div className="p-3 pt-0 border-t border-os-pewter bg-os-panel space-y-2 text-[11px]">
                    {trace.thought && (
                      <div>
                        <span className="text-os-ash font-semibold uppercase text-[10px]">Thinking:</span>
                        <p className="text-os-fog font-sans mt-0.5 leading-relaxed bg-os-void p-2 rounded-input border border-os-pewter">
                          {trace.thought}
                        </p>
                      </div>
                    )}
                    {trace.observation && (
                      <div>
                        <span className="text-os-clear font-semibold uppercase text-[10px]">Observation & Finding:</span>
                        <p className="text-os-clear font-sans mt-0.5 leading-relaxed bg-os-raised p-2 rounded-input border border-os-clear">
                          {trace.observation}
                        </p>
                      </div>
                    )}
