  ...
  useEffect(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      particles.push({
        ...
        color: Math.random() > 0.4 ? '#38bdf8' : Math.random() > 0.5 ? '#22d3ee' : '#34d399',
    ...
    const render = () => {
      ...
      for (const v of vessels) {
        ...
        if (v.trail.length > 1) {
          ...
          ctx.strokeStyle = v.type === 'Autonomous' ? 'rgba(52, 211, 153, 0.35)' : 'rgba(56, 189, 248, 0.3)';
          ctx.lineWidth = 1.5;
          ctx.setLineDash([4, 4]);
          ctx.stroke();
          ctx.setLineDash([]);
        ...
        const color = isAutonomous ? '#34d399' : '#38bdf8';
