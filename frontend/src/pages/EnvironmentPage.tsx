...
export const EnvironmentPage: React.FC = () => {
 ...
 const reading = (label: string, value: string, hint?: string) => (
    <div key={label} className="flex flex-col gap-0.5 px-3 py-2 rounded-row bg-os-void/80 border border-os-steel">
 ...
 eventHandlers={{ click: () => setSelectedId(v.id) }}>
        ...
        {sidebarOpen ? (
          <div className="absolute left-4 top-4 bottom-4 z-[1000] flex flex-col pointer-events-none os-reveal">
            <div className="w-[350px] h-full bg-os-panel border border-os-steel rounded-panel flex flex-col pointer-events-auto overflow-hidden">
 ...
 className="bg-os-void text-white text-xs font-mono border border-os-steel rounded-input px-3 py-2.5 focus:outline-none focus:border-os-silver"
                  >
                    {vessels.map(v => <option key={v.id} value={v.id} className="bg-[#15171b] text-white">{v.name}</option>)}
                  </select>
                </label>
                {selected && (
                  <div className="px-3 py-1.5 rounded-input bg-os-void border border-os-steel">
                ...
                {storms.length > 0 && (
                  <div className="flex flex-col gap-2.5 p-3.5 rounded-row bg-os-raised border border-risk-critical">
                    ...
                    <div className="text-xs text-os-fog font-mono flex flex-col gap-1">
                      <span>Winds: <b className="text-risk-critical font-bold">{storms[0].wind_speed_knots} knots</b></span>
                      <span>Buffer Radius: <b className="text-risk-critical font-bold">{storms[0].radius_km} km</b></span>
 ...
 className="absolute left-4 top-4 z-[1000] px-4 py-2 rounded-row bg-os-panel border border-os-steel text-white text-xs font-bold font-mono flex items-center gap-2 hover:bg-os-raised transition"
        ...
        <div className="absolute right-4 top-4 bottom-4 z-[1000] flex flex-col pointer-events-none os-reveal">
          <div className="w-[370px] h-full bg-os-panel border border-os-steel rounded-panel flex flex-col pointer-events-auto overflow-hidden">
            <div className="px-5 py-3.5 border-b border-os-steel flex items-center justify-between bg-os-raised">
            ...
            <div className="flex-1 overflow-y-auto os-scrollbar p-5 flex flex-col gap-3">
              {alerts.map((a, i) => (
                <div key={a.id} className="flex flex-col gap-2 p-3 rounded-row bg-os-void/70 border border-os-steel">
                  <div className="flex items-center gap-2">
              ...
              {alerts.length === 0 && (
                <div className="p-4 rounded-row bg-os-void/60 border border-os-steel text-center">
