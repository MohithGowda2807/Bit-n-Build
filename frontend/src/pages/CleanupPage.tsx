...
export const CleanupPage: React.FC = () => {
 ...
 return (
    <div className="relative w-full h-[calc(100vh-4rem)] flex overflow-hidden bg-os-void font-sans">
 ...
 className="bg-os-panel border border-os-pewter rounded-row px-3 py-2 text-xs font-mono text-white hover:text-white hover:border-os-silver transition flex items-center gap-2 cursor-pointer"
          >
 ...
 title="Generate Stage 4 Autonomous Sortie Order"
        ...
        {sidebarOpen && (
          <div className="absolute left-4 top-16 bottom-4 w-84 z-[1000] flex flex-col space-y-3 pointer-events-none">
            <div className="bg-os-panel border border-os-pewter rounded-panel p-4 flex flex-col max-h-full overflow-hidden pointer-events-auto">
              {/* Studio Tabs */}
              <div className="flex items-center space-x-1 p-1 bg-os-void rounded-row border border-os-pewter mb-3 font-mono text-xs">
                <button
 onClick={() => setSidebarTab('debris')}
 className={`flex-1 py-1.5 rounded-input font-bold transition ${
 sidebarTab === 'debris'
 ...
 d.severity >= 85
                                ? 'bg-risk-critical/20 text-risk-critical border border-risk-critical'
                                : 'bg-risk-moderate/20 text-risk-moderate border border-risk-moderate'
                            }`}
                          >
                            {d.clean_up_priority}
                          </span>
                        </div>
                        <div className="flex items-center justify-between text-[11px] text-os-ash mt-1">
                          <span>Severity: {d.severity}/100</span>
                          <span className="text-os-signal font-semibold">
